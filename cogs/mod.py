...
class Moderation(commands.Cog):
    ...
    @commands.command(
      name="kick",
      help="Принести члена культа в жертву Хозяину."
    )
    @commands.guild_only()
    ...
    async def kick_(self, ctx : commands.Context, member: discord.Member, *,   reason: str = None):
      ...
      try:
          ...
          embed.set_footer(text=f"{ctx.author} взывает к Хозяину")
          embed.set_author(name=f"✅ {member.name} был поглощён", icon_url=member.avatar_url)
          await ctx.send(embed=embed)
          await member.send(f"Вы были принесены в жертву на сервере **{ctx.guild.name}** за **{reason}** пользователем **{ctx.author}**")
    ...
    async def unban_(self, ctx, member: MemberID, *, reason: str = None):
        ...
        try:
            ...
            embed.set_footer(text=f"{ctx.author} взывает к Хозяину")
            embed.set_author(name=f"✅ был прощён Хозяином!")
            await ctx.send(embed=embed)
